    ...
    function () {
      async function fetchLiveFlightPrice() {
        ...
        try {
          ...
          setLiveFlightPrice(data.pricePerPerson);
        ...
        {step === 4 && selectedDest && estimate && (
          <div style={{ maxWidth: 600 }}>
            ...
            <div style={{ background: 'var(--accent)', borderRadius: 16, padding: '24px 28px', marginBottom: 20, color: '#fff' }}>
              ...
              <div style={{ fontFamily: 'var(--font-serif)', fontSize: 48, lineHeight: 1 }}>{fmt(displayedTripTotal)}</div>
              <div style={{ fontSize: 13, opacity: 0.7, marginTop: 8 }}>
                {fmt(displayedPerPerson)} per person, {liveFlightPrice ? 'flight price is live' : 'prices are estimates'}
              </div>
            </div>
            ...
            <div style={{ background: '#fff', border: '1px solid #E8E6DF', borderRadius: 14, padding: '20px 24px', marginBottom: 20 }}>
              ...
              {[
                {
                  label: '✈️ Flights (' + estimate.trip.travelers + 'x ' + fmt(displayedFlightPrice) + ')' + (liveFlightPrice ? ' - live' : ' - estimate'),
                  value: displayedFlightTotal,
                },
                { label: '🏨 Hotel (' + estimate.trip.nights + ' nights)', value: displayedHotelTotal },
                { label: '🍽️ Food and drink', value: displayedFoodTotal },
                { label: '🚌 Local transport', value: displayedTransportTotal },
                { label: '🎟️ Activities', value: displayedActivitiesTotal },
                { label: '🛡️ Safety buffer (8%)', value: displayedSafetyBuffer },
              ].map(function (row) {
                return (
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', padding: '14px 0 0', fontSize: 16, fontWeight: 600 }}>
                <span>Total</span>
                <span style={{ color: 'var(--accent)' }}>{fmt(displayedTripTotal)}</span>
              </div>
              ...
              {liveFlightMeta && (
                <p style={{ fontSize: 12, color: '#666', marginTop: 10 }}>
                  Live source: {liveFlightMeta.provider}. Price field: {liveFlightMeta.pricePath}. Results: {liveFlightMeta.rawCount}.
                </p>
              )}

              {liveFlightError && (
                <p style={{ fontSize: 12, color: '#dc2626', marginTop: 10 }}>
                  Could not fetch live flight price. Showing estimate instead. Reason: {liveFlightError}
                </p>
              )}

              {liveFlightFetchedAt && (
                <p style={{ fontSize: 12, color: '#666', marginTop: 10 }}>
                  Live flight price updated: {new Date(liveFlightFetchedAt).toLocaleString()}
                </p>
              )}
            </div>
