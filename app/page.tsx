...
export default function Home() {
  ...
  const [liveFlightError, setLiveFlightError] = useState('');
  const [liveFlightFetchedAt, setLiveFlightFetchedAt] = useState('');
  const [liveFlightMeta, setLiveFlightMeta] = useState<LiveFlightMeta | null>(null);
  ...
  useEffect(
    function () {
      async function fetchLiveFlightPrice() {
        if (step !== 4 || !selectedDest || !startDate || !endDate) {
          return;
        }

        setLiveFlightLoading(true);
        setLiveFlightError('');
        setLiveFlightPrice(null);
        setLiveFlightFetchedAt('');
        setLiveFlightMeta(null);

        try {
          const response = await fetch('/api/live-flight', {
            method: 'POST',
            headers: {
            ...
            body: JSON.stringify({
              origin: origin || 'CPH',
              destination: selectedDest.airportCode,
          ...
          if (!response.ok) {
            const messageParts = [data.error];

            if (data.status) {
              messageParts.push('Status: ' + data.status);
            }

            if (data.details) {
              messageParts.push('Details: ' + String(data.details).slice(0, 300));
            }

            if (data.rawCount !== undefined) {
              messageParts.push('Raw count: ' + data.rawCount);
            }
          ...
          const apiTotalFlightPrice = Number(data.totalFlightPrice);
          const apiPricePerPerson = Number(data.pricePerPerson);

          if (
            Number.isFinite(apiTotalFlightPrice) &&
            apiTotalFlightPrice > 0 &&
            travelers > 0
          ) {
            setLiveFlightPrice(Math.round(apiTotalFlightPrice / travelers));
          } else if (Number.isFinite(apiPricePerPerson) && apiPricePerPerson > 0) {
            setLiveFlightPrice(Math.round(apiPricePerPerson));
          } else {
            throw new Error('API returned no valid totalFlightPrice or pricePerPerson.');
          }
    ...
    [step, selectedDest, startDate, endDate, origin, travelers]
  );

  const displayedFlightPrice = liveFlightPrice ?? estimate?.flight.price ?? 0;

  const displayedFlightTotal = estimate
    ? displayedFlightPrice * estimate.trip.travelers
    : 0;

  const displayedHotelTotal = estimate ? estimate.trip.hotelTotal : 0;
  const displayedFoodTotal = estimate ? estimate.trip.foodTotal : 0;
  const displayedTransportTotal = estimate ? estimate.trip.transportTotal : 0;
  const displayedActivitiesTotal = estimate ? estimate.trip.activitiesTotal : 0;

  const displayedSubtotal =
    displayedFlightTotal +
    displayedHotelTotal +
    displayedFoodTotal +
    displayedTransportTotal +
    displayedActivitiesTotal;

  const displayedSafetyBuffer = estimate
    ? Math.round(displayedSubtotal * 0.08)
    : 0;

  const displayedTripTotal = displayedSubtotal + displayedSafetyBuffer;
      ...
      {step === 1 && (
        ...
        >
          ...
          >
            ...
            >
              Pick a travel theme, choose a destination, and instantly see a full
              breakdown of flights, hotels, food and activities.
            </p>
            ...
            >
              ...
              {[
                ...
                },
                {
                  ...
                  value: displayedHotelTotal,
                },
                {
                  label: '🍽️ Food and drink',
                  value: displayedFoodTotal,
                },
                {
                  label: '🚌 Local transport',
                  value: displayedTransportTotal,
                },
                {
                  label: '🎟️ Activities',
                  value: displayedActivitiesTotal,
                },
                {
                  label: '🛡️ Safety buffer (8%)',
                  value: displayedSafetyBuffer,
                },
              ].map(function (row) {
                return (
                  <div
              ...
              {liveFlightError && (
                <p style={{ fontSize: 12, color: '#dc2626', marginTop: 10 }}>
                  Could not fetch live flight price. Showing estimate instead.
                  Reason: {liveFlightError}
                </p>
              )}

              {liveFlightFetchedAt && (
                <p style={{ fontSize: 12, color: '#666', marginTop: 10 }}>
                  Live flight price updated:{' '}
                  {new Date(liveFlightFetchedAt).toLocaleString()}
                </p>
              )}
            </div>
                ...
                <div>
                  ...
                  <div style={{ display: 'flex', gap: 8 }}>
                    {([
                      ...
                      ['cheapest', 'Cheapest'],
                      ['fastest', 'Fastest'],
                    ] as [FlightSort, string][]).map(function (opt) {
                ...
                <div>
                  ...
                  <div style={{ display: 'flex', gap: 8 }}>
                    {([
                      ...
                      ['direct', 'Direct only'],
                      ['1stop', '1 stop max'],
                    ] as [StopsFilter, string][]).map(function (opt) {
